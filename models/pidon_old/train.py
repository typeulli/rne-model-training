"""Train PiDoNOld on laser-scan temperature fields with PINN losses.

Invoked through the top-level dispatcher::

    python train.py pidon_old --iterations 20000
...
from .dataset import PiDoNOldDataset
from .loss import (
    PROPERTIES,
    LossWeights,
    PINNLoss,
...
@torch.no_grad()
def evaluate(
    model: PiDoNOld, split: SimulationDataset, chunk: int = 65536
) -> tuple[float, float]:
...
def build_model(
    ...
    return PiDoNOld(**architecture), architecture
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="defaults to checkpoints/pidon_old/<run-name>.pt",
    )
