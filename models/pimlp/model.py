"""The MLP baseline with the physics put back in.

The same dense stack as :mod:`models.mlp` over the same five inputs, but fitted
against the heat equation as well as the labels. :mod:`models.pimlp.loss` needs
``dT/dt`` and the Laplacian, so this network exposes :meth:`PhysicsMLP.derivatives`
where :class:`~models.mlp.model.SimpleMLP` has nothing at all.

That is also why :meth:`forward` takes ``(laser_power, coords)`` instead of the
single ``[batch, 5]`` block ``SimpleMLP`` takes: the residuals are written against
``(x, y, z, t)``, so autograd needs a tensor of exactly those four columns to
differentiate with respect to. The two are concatenated into the
``(P, t, z, y, x)`` order the stack expects inside :meth:`forward`, and the chain
rule is carried through that concatenation and through the normalisation alike --
...
class FieldDerivatives:
    ...
    when :meth:`PhysicsMLP.derivatives` is called with ``second_order=False``.
    """
...
class PhysicsMLP(nn.Module):
    """``T_hat(x, y, z, t; P)`` as a dense stack, trained against the PDE.

    ``input_mean`` and ``input_scale`` are ``[5]`` in the ``(P, t, z, y, x)``
    column order the stack is fed in; ``temperature_offset`` and
    ``temperature_scale`` map the network's O(1) output back to Kelvin. All of
    them are buffers, so they ride along in the state dict and a checkpoint
    ...
    def __init__(
        ...
        """See the class docstring for the normalisation. ``gaussian_exponent_scale``
        is not consumed by :meth:`forward` -- it has no effect on the network itself --
        it is carried here only so the laser-source sharpness used to *train* this
        checkpoint (see ``models/pimlp/loss.laser_flux``) travels with the checkpoint
        instead of living as a separate, easy-to-lose CLI flag.
        """
        super().__init__()
    ...
    def forward(self, laser_power: Tensor, coords: Tensor) -> Tensor:
        """Predict temperature at ``coords`` for the process parameter ``laser_power``.

        ``laser_power`` is ``[batch, 1]`` (a batch size of 1 is broadcast over the
        query points) and ``coords`` is ``[batch, 4]`` holding ``(x, y, z, t)`` in
        physical units. Returns ``[batch, 1]`` in the same temperature unit as
        ``temperature_offset``.
        """
        ...
        if laser_power.dim() != 2 or laser_power.size(-1) != 1:
            raise ValueError(
                f"laser_power must have shape [batch, 1], got {tuple(laser_power.shape)}"
            )
        if laser_power.size(0) == 1 and coords.size(0) != 1:
            laser_power = laser_power.expand(coords.size(0), -1)
        elif laser_power.size(0) != coords.size(0):
            raise ValueError(
        ...
        inputs = torch.cat((laser_power, t, z, y, x), dim=-1)
