...
from .model import FieldDerivatives, PhysicsMLP
...
def laser_flux(coords: Tensor, power: Tensor, exponent_scale: float = 1.0) -> Tensor:
...
class PointSet:
    ...
    ``laser_power`` is the process parameter and ``coords`` the space-time point
    ``(x, y, z, t)``. The remaining fields are only needed by some terms:
    ``temperature`` by the data loss, ``normal`` by the Neumann boundary losses,
    and ``q_laser`` by the top-surface loss.
    """
