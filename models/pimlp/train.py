"""Train PiMLP on laser-scan temperature fields with PINN losses.
...
store millimetres and are converted by :mod:`dataset` on load. ``P`` is one of
the five network inputs and is constant within a file, so the model only has
something to learn about it once several files at different powers are present;
every file under the data directory is globbed and concatenated automatically.
"""
...
@torch.no_grad()
def evaluate(
    model: PhysicsMLP, split: SimulationDataset, chunk: int = 65536
) -> tuple[float, float]:
...
def build_model(
    ...
    return PhysicsMLP(**architecture), architecture
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="defaults to checkpoints/pimlp/<run-name>.pt",
    )
