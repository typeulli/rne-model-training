"""Train the MLP baseline on ``(P, t, z, y, x) -> T``.
...
@torch.no_grad()
def evaluate(
    model: SimpleMLP, inputs: Tensor, target: Tensor, chunk: int = 65536
) -> tuple[float, float]:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint", type=Path, default=None, help="defaults to checkpoints/mlp/<run-name>.pt"
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    sampler = MLPDataset(train_split, generator)
    val_inputs, val_target = MLPDataset(val_split, generator).all()
    ...
    model = SimpleMLP(**architecture).to(device=device, dtype=dtype)
