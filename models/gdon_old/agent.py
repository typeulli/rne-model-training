"""GDoNOld behind the shared :class:`~agent.BaseAgent` inference contract.
...
class GDoNOldAgent(BaseAgent):
    """Wraps a trained :class:`~models.gdon_old.model.GDoNOld` for inference."""
...
def build_agent(
    ...
    model = GDoNOld(**payload["architecture"])
    model.load_state_dict(payload["model"])  # normalisation buffers ride along
    return GDoNOldAgent(model, payload["bounds"], shape=shape, device=device)
