"""GDoNOld -- GPiDoNOld with the physics taken out.

Same architecture as :mod:`models.gpidon_old`: a DeepONet whose inner product is
...
second derivative and :meth:`GPiDoNOld.derivatives` has no counterpart; the network
is fitted to labelled temperatures alone.

That makes GDoNOld the controlled comparison for GPiDoNOld: same map, same gate, same
...
class GDoNOld(nn.Module):
    ...
    ``g`` is :func:`~models.gdon_old.laser.beam_gaussian`, a unit-peak Gaussian riding
    ...
    *unrepresentable*. With ``G = g + p`` the operator still gets the beam's shape
    ...
    The activation defaults to ``tanh`` to match :class:`~models.gpidon_old.model.GPiDoNOld`,
