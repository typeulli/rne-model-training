"""Train GDoNOld on laser-scan temperature fields with a supervised loss alone.

Invoked through the top-level dispatcher::

    python train.py gdon_old --iterations 20000

Identical to ``models/gpidon_old/train.py`` except that the objective is a scaled MSE
...
from .dataset import GDoNOldDataset
...
@torch.no_grad()
def evaluate(
    model: GDoNOld, split: SimulationDataset, chunk: int = 65536
) -> tuple[float, float]:
...
def build_model(
    ...
    return GDoNOld(**architecture), architecture
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="defaults to checkpoints/gdon_old/<run-name>.pt",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    sampler = GDoNOldDataset(train_split, generator)
