"""Geometry of the moving laser beam, used to gate the operator output.

A copy of ``models/gpidon_old/laser.py`` minus the source term: GDoNOld has no boundary
condition to write, so the only thing it needs from the beam is where it is.
Kept as a copy rather than an import so an experiment on one model cannot shift
