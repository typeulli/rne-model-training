"""Train GMLP on ``(P, t, z, y, x) -> T``, with the Gaussian gate on the output.

Invoked through the top-level dispatcher::

    python train.py gmlp --epochs 20 --batch-size 8192

Identical to ``models/mlp/train.py`` -- same loop, same objective, same
hyperparameters -- except that the network carries the Gaussian gate and hence a
``--gaussian-exponent-scale``. Everything else is held fixed so the two runs can
be compared directly.

The whole corpus is loaded into memory up front, split once into train and
validation, and every step draws a random batch (with replacement) from the train
split. An "epoch" is not a shuffled pass over every row; it is
...
@torch.no_grad()
def evaluate(
    model: GatedMLP, inputs: Tensor, target: Tensor, chunk: int = 65536
) -> tuple[float, float]:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint", type=Path, default=None, help="defaults to checkpoints/gmlp/<run-name>.pt"
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    sampler = GMLPDataset(train_split, generator)
    val_inputs, val_target = GMLPDataset(val_split, generator).all()
    ...
    model = GatedMLP(**architecture).to(device=device, dtype=dtype)
