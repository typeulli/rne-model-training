"""Reorder the shared corpus into the ``(t, z, y, x)`` columns CJMLP expects.

A copy of :mod:`models.cgmlp.dataset`: the gate lives inside the network and is
built from the coordinate columns of the input, so swapping the Gaussian for a
moving-source field changes nothing about how batches are drawn.
