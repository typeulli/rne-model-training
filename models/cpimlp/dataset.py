...
class CPiMLPDataset:
    ...
    def initial(self, count: int) -> PointSet:
        ...
        The rows arrive carrying a power, and it is dropped here for the reason
        every other batch drops it: the network has no input to receive it. The
        seven powers therefore contribute seven different targets at the same
        point, exactly as they do in :meth:`data`.
        """
        coords, _power, temperature = self.initial_slice.sample(count, self.generator)
        return PointSet(coords=coords, temperature=temperature)
