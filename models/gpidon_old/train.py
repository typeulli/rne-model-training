"""Train GPiDoNOld on laser-scan temperature fields with PINN losses.

Invoked through the top-level dispatcher::

    python train.py gpidon_old --iterations 20000
...
from .dataset import GPiDoNOldDataset
from .loss import (
    PROPERTIES,
    LossWeights,
    PINNLoss,
...
@torch.no_grad()
def evaluate(
    model: GPiDoNOld, split: SimulationDataset, chunk: int = 65536
) -> tuple[float, float]:
...
def build_model(
    ...
    return GPiDoNOld(**architecture), architecture
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="defaults to checkpoints/gpidon_old/<run-name>.pt",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    sampler = GPiDoNOldDataset(train_split, generator, domain=domain)
    ...
    if args.align_every > 0:
        probe_sampler = GPiDoNOldDataset(
            train_split,
