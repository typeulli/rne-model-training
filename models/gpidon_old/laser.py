"""Geometry of the moving laser beam, shared by the network and its objective.

``model.py`` gates the operator output by a Gaussian centred on the beam, and
``loss.py`` builds the top-surface source term from the same beam -- but
``loss.py`` already imports ``model.py``, so the geometry cannot live in either
without a cycle. It lives here instead.

``BEAM_RADIUS``, ``LASER_START_X``, ``LASER_Y`` and ``SCAN_SPEED`` are recovered
from the data by ``python calibrate.py``; re-run it and paste the block back
...
def beam_gaussian(coords: Tensor, exponent_scale: Tensor | float = 1.0) -> Tensor:
    ...
    with ``k`` the ``exponent_scale``. Unlike :func:`~models.gpidon_old.loss.laser_flux`
    this decays with depth as well, since it gates a volumetric field rather than a
    surface flux: it is 1 at the beam centre on the top face and falls off in every
    direction away from it. ``k > 1`` tightens that envelope, ``k < 1`` widens it;
    the peak is 1 either way because ``exp(0) = 1`` regardless of ``k``.
    """
