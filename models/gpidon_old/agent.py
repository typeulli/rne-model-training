"""GPiDoNOld behind the shared :class:`~agent.BaseAgent` inference contract.
...
class GPiDoNOldAgent(BaseAgent):
    """Wraps a trained :class:`~models.gpidon_old.model.GPiDoNOld` for inference."""
...
def build_agent(
    ...
    model = GPiDoNOld(**payload["architecture"])
    model.load_state_dict(payload["model"])  # normalisation buffers ride along
    return GPiDoNOldAgent(model, payload["bounds"], shape=shape, device=device)
