"""The PINN objective for GPiDoNOld, and the physical constants it is built from.

The residuals need material properties and laser parameters that the ``.npy``
files do not carry. The material ones live here, next to the terms that consume
them, and are recovered from the data by ``python calibrate.py`` -- re-run it
and paste the block back whenever the contents of the data directory change.
The beam geometry is shared with the network and lives in :mod:`.laser`.
...
from .laser import BEAM_RADIUS, LASER_Y, beam_centre_x
from .model import FieldDerivatives, GPiDoNOld
...
# top-surface energy balance closes to 0.86% of peak flux. The beam geometry
# fitted alongside it lives in `laser.py`.
# ---------------------------------------------------------------------------
...
def laser_flux(coords: Tensor, power: Tensor) -> Tensor:
    ...
    with the beam centre ``x_l(t) = LASER_START_X + SCAN_SPEED * t``. This is the
    physical source the top boundary condition is written against, so it stays the
    calibrated Gaussian; the network's own envelope in :func:`~models.gpidon_old.laser.beam_gaussian`
    is what ``gaussian_exponent_scale`` reshapes.
    """
    x, y, t = coords[:, 0:1], coords[:, 1:2], coords[:, 3:4]
    squared_distance = (x - beam_centre_x(t)) ** 2 + (y - LASER_Y) ** 2
    return peak_laser_flux(power) * torch.exp(
        -2.0 * squared_distance / BEAM_RADIUS**2
    )
