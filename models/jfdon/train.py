"""Train JFDoN: a DeepONet whose answer is a temperature *history*.

Invoked through the top-level dispatcher::

    python train.py jfdon --iterations 140000 --hidden 64 64 64 --latent-dim 128
...
**How it differs from ``jdon``.** The trunk takes ``(x, y, z)`` and no clock;
...
0.4 s apart. ``jdon`` is continuous in ``t`` because ``t`` goes into its trunk;
